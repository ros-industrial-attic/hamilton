...
namespace Hamilton
{
  ...
  const std::string SPARSE = "sparse"; 
  const double CONST_TOLERANCED_CP_1 = 0.0001;
  ...
  * Is a member of HybridPlanner class and allows to define a few parameters required by MoveIt! and Descartes*/
  struct HybridConfiguration
  {
    ...
    std::string world_frame;       // = (typically) "base_link" OR "world_frame"; /* The name of the world link in the URDF file, the frame in which you are expressing poses. */
  ...
  class HybridPlanner
  {
    public:
      ...
      // TODO extend to other types - joints, descartes types, etc.
    	bool appendFreeMotionTrajectorySegment(std::vector<geometry_msgs::Pose>& waypoints);
    	bool appendProcessPathTrajectorySegment(std::vector<geometry_msgs::Pose>& waypoints);

      // Appends unplanned free space segment, defined by start and end point only as a TrajectorySegment object. 
      bool appendFreeMotionTrajectorySegment(geometry_msgs::Pose start_pose, geometry_msgs::Pose end_pose);
      ...
      // TODO Not Implemented: Makes a motion planning request if the TrajetorySegemnt.type == FREE_MOTION_START_END_ONLY
    	void planHybrid();

      //====== Helper functions =====//
      /* Create an axial trajectory pt from a given tf transform */
      static descartes_core::TrajectoryPtPtr tfToAxialTrajectoryPt(const tf::Transform& nominal, double discretization);  // TODO not implemented/ not needed right now
      ...
      static descartes_core::TrajectoryPtPtr makeTolerancedCartesianPoint(const Eigen::Affine3d& pose);
